...
import { PurchaseStep, useMysteryBoxStore } from "@/stores/mysteryBoxStore";
...
const ProcessingStep: React.FC<ProcessingStepProps> = ({
  ...
  const { finishProcessing, rewards, transactionHash, usdcReceived } =
    useMysteryBoxStore();
  ...
  useEffect(() => {
    ...
    // Auto-transition to finished step after 20 seconds
    const finishTimer = setTimeout(() => {
      finishProcessing(rewards, usdcReceived || undefined);
    }, duration);
  ...
  }, [finishProcessing, rewards, usdcReceived]);
